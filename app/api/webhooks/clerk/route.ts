...
async function seedCalendarItems(clerkId: string) {
  const [{ itemCount }] = await db
    ...
    .from(calendarItems)
    .where(eq(calendarItems.clerkId, clerkId));
  ...
  await db.insert(calendarItems).values([
    {
      ...
      notes: 'Align the next product milestone with the launch narrative.',
      kind: 'task',
      category: 'work',
    ...
    },
    {
      ...
      notes: 'Bring the weekly priorities and blockers.',
      kind: 'reminder',
      category: 'meeting',
    ...
    },
    {
      ...
      notes: 'Capture decisions from the current sprint.',
      kind: 'task',
      category: 'work',
    ...
    },
    {
      ...
      notes: 'A short reset between focused sessions.',
      kind: 'reminder',
      category: 'health',
    ...
    {
      clerkId,
      title: 'Prepare study outline',
      notes: 'Save this for the next open planning block.',
      kind: 'task',
      category: 'study',
      scheduledDate: null,
      scheduledTime: null,
    ...
    {
      clerkId,
      title: 'Collect whiteboard ideas',
      notes: 'Turn loose notes into the next working session.',
      kind: 'task',
      category: 'personal',
      scheduledDate: null,
      scheduledTime: null,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (event.type === 'user.created' || event.type === 'user.updated') {
      ...
      await db
        .insert(users)
        .values({ clerkId: data.id, ...profile })
        .onConflictDoUpdate({ target: users.clerkId, set: profile });
    ...
    if (event.type === 'user.deleted' && event.data.id) {
      ...
      const boards = await db
        ...
        .from(kanbanBoards)
        .where(eq(kanbanBoards.clerkId, clerkId));
