...
function boardIdFromRoom(roomId: unknown) {
  const match = typeof roomId === 'string' && /^flowbase:kanban:(\d+)$/.exec(roomId);
...
export async function POST(request: Request) {
  try {
    ...
    if (!boardId) return NextResponse.json({ error: 'Invalid room.' }, { status: 400 });
    ...
    const [membership] = await db
      ...
      .where(and(eq(kanbanBoardCollaborators.boardId, boardId), eq(kanbanBoardCollaborators.clerkId, userId)));
    ...
    return NextResponse.json({ error: 'Could not authorize collaboration.' }, { status: 500 });
  }
}
