...
async function getPage(spaceId: number, pageId: number) {
  const [page] = await db
    .select()
    .from(spacePages)
    .where(and(eq(spacePages.id, pageId), eq(spacePages.spaceId, spaceId)));
  return page;
}
...
export async function GET(
  ...
  const spaceId = validId(spaceValue);
  const pageId = validId(pageValue);
  if (!spaceId || !pageId)
    return NextResponse.json({ error: 'Invalid page.' }, { status: 400 });
  const space = await getAccessibleSpace(spaceId, userId);
  const page = await getPage(spaceId, pageId);
  if (!space || !page)
...
export async function PATCH(
  ...
  if ('title' in body) {
    ...
    if (!title)
      return NextResponse.json(
        { error: 'A page title is required.' },
        { status: 400 },
      );
    updates.title = title;
  }
  ...
  if ('template' in body) {
    if (
      !PAGE_TEMPLATES.includes(body.template as (typeof PAGE_TEMPLATES)[number])
    )
      return NextResponse.json(
  ...
  if ('spaceId' in body) {
    const destinationId =
      typeof body.spaceId === 'number' ? body.spaceId : Number(body.spaceId);
    if (
      !Number.isInteger(destinationId) ||
      !(await getAccessibleSpace(destinationId, userId))
    )
      return NextResponse.json(
        { error: 'Destination space not found.' },
        { status: 400 },
      );
    updates.spaceId = destinationId;
  }
  const [item] = await db
    .update(spacePages)
    .set(updates)
