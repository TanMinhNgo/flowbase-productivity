...
export async function GET(
  ...
  const [membership] = await db
    ...
    .where(
      and(
        eq(kanbanBoardCollaborators.boardId, board.id),
  ...
  const collaborators = await db
    .select({
      clerkId: kanbanBoardCollaborators.clerkId,
      name: users.name,
      email: users.email,
...
export async function POST(
  ...
  if (!board)
    return NextResponse.json({ error: 'Board not found.' }, { status: 404 });
  if (board.clerkId !== userId)
    return NextResponse.json(
      { error: 'Only the board owner can invite collaborators.' },
      { status: 403 },
    );
  ...
  if (!invitee)
    return NextResponse.json(
      {
        error:
          'This person needs a Flowbase account before they can be invited.',
      },
      { status: 404 },
    );
  if (invitee.clerkId === userId)
    return NextResponse.json(
  ...
  await db
    ...
    .values({ boardId: board.id, clerkId: invitee.clerkId })
    .onConflictDoNothing();
  await ensureBoardRoom(board);
  await getLiveblocks().updateRoom(`flowbase:kanban:${board.id}`, {
    usersAccesses: { [invitee.clerkId]: ['*:write'] },
  });
  return NextResponse.json({
    collaborator: {
      clerkId: invitee.clerkId,
      name: invitee.name,
      email: invitee.email,
