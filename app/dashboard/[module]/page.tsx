...
export default async function ModulePage({
  ...
  return (
    <div className="animate-[rise_500ms_cubic-bezier(0.16,1,0.3,1)_both]">
      <span
        className={`grid size-12 place-items-center rounded-2xl ${config.tone}`}
      >
        <Icon size={23} strokeWidth={1.8} />
      </span>
      <h1 className="mt-7 text-3xl font-semibold tracking-[-0.055em] text-[#1d2635] sm:text-4xl dark:text-[#f6f8fc]">
        {config.title}
      </h1>
      ...
      <section className="mt-10 grid min-h-72 place-items-center rounded-2xl border border-dashed border-[#cedaeb] bg-white/65 p-8 text-center dark:border-[#41516a] dark:bg-white/3">
        <div>
          ...
          <p className="mt-2 max-w-md text-[13px] leading-6 text-[#738094] dark:text-[#aab6c7]">
            The navigation and protected workspace shell are in place. This
            module will gain its full product experience in the next build step.
          </p>
        </div>
      </section>
    </div>
