...
const COLORS: Record<Color, string> = {
  coral: 'text-[#e26748] bg-[#fff0eb]',
  apricot: 'text-[#bd6d1e] bg-[#fff3e3]',
  rose: 'text-[#c24b72] bg-[#fff0f4]',
  violet: 'text-primary bg-primary/10',
  sky: 'text-[#2d83ae] bg-[#eef8ff]',
  mint: 'text-[#23816a] bg-[#eafaf5]',
};
  ...
  return (
    <div className="flex -space-x-2">
      {members.slice(0, 3).map((member) => (
        <span
          key={member.clerkId}
          title={member.name ?? 'Flowbase member'}
          className="grid size-7 place-items-center overflow-hidden rounded-full border-2 border-card bg-primary/15 text-[10px] font-semibold text-primary"
        >
          {member.imageUrl ? (
            <img
              src={member.imageUrl}
      ...
      {members.length > 3 ? (
        <span className="grid size-7 place-items-center rounded-full border-2 border-card bg-secondary text-[10px] font-semibold">
          +{members.length - 3}
        </span>
      ) : null}
    </div>
    ...
    <div className="fixed inset-0 z-50 grid place-items-center p-4">
      ...
      <form
        onSubmit={(event) => {
          ...
          setSaving(true);
          void onCreate(name, description, color)
            .catch((reason) =>
              setError(
      ...
      >
        ...
        <div className="mt-4">
          ...
          <div className="mt-2 flex gap-2">
            {(Object.keys(COLORS) as Color[]).map((entry) => (
              <button
                key={entry}
                type="button"
                onClick={() => setColor(entry)}
                className={`size-8 rounded-full ${COLORS[entry]} ${color === entry ? 'ring-2 ring-primary ring-offset-2' : ''}`}
                aria-label={entry}
              />
            ))}
          </div>
...
export function SpacesWorkspace() {
  ...
  const visible = useMemo(
    () =>
      spaces
        .filter((space) => {
          const match = `${space.name} ${space.description}`
            .toLowerCase()
          ...
          const filter =
            tab === 'all'
              ? !space.archivedAt
              : tab === 'favorites'
                ? space.isFavorite && !space.archivedAt
                : tab === 'recent'
                  ? Boolean(space.lastOpenedAt) && !space.archivedAt
    ...
    <section className="mx-auto max-w-350 animate-[rise_500ms_cubic-bezier(0.16,1,0.3,1)_both] pb-10">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {spaces.filter((space) => !space.archivedAt).length} spaces
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="mt-7 flex flex-wrap items-center gap-3">
        <label className="relative min-w-[min(100%,20rem)] flex-1">
          ...
          <input
            ...
            className="h-10 w-full rounded-xl border border-border bg-card pl-9 pr-3 text-sm outline-none focus:ring-2 focus:ring-primary/20"
          />
        </label>
        <div className="flex rounded-xl border border-border bg-card p-1">
          <button
            ...
            className={`grid size-8 place-items-center rounded-lg ${view === 'grid' ? 'bg-secondary text-primary' : 'text-muted-foreground'}`}
          >
            <Grid2X2 size={16} />
          </button>
          <button
            ...
            className={`grid size-8 place-items-center rounded-lg ${view === 'list' ? 'bg-secondary text-primary' : 'text-muted-foreground'}`}
          >
            <LayoutList size={16} />
          </button>
        </div>
      ...
      <div className="mt-5 flex gap-1 overflow-x-auto border-b border-border">
        {[
          ...
          <button
            ...
            className={`shrink-0 border-b-2 px-3 py-2 text-sm font-medium ${tab === value ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
          >
            {label}
          </button>
        ))}
      ...
      {visible.length ? (
        <div
          className={`mt-6 ${view === 'grid' ? 'grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3' : 'space-y-2'}`}
        >
          {visible.map((space) => (
            <article
              key={space.id}
            ...
            >
              ...
              />
              <span
                className={`relative grid size-11 place-items-center rounded-xl ${COLORS[space.color]}`}
              >
                <Folder size={22} />
              </span>
              <div
                className={`relative mt-5 ${view === 'list' ? 'mt-0 flex-1' : ''}`}
              >
                <div className="flex items-start gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      void update(space, { isFavorite: !space.isFavorite })
                    }
                    className="relative z-10 grid size-7 place-items-center rounded-lg text-muted-foreground hover:bg-secondary"
                  >
                    <Star
                      size={16}
                      fill={space.isFavorite ? 'currentColor' : 'none'}
                      className={space.isFavorite ? 'text-primary' : ''}
                    />
                  </button>
                  <button
                    type="button"
        ...
        <div className="mt-8 grid min-h-80 place-items-center rounded-2xl border border-dashed border-border bg-card p-8 text-center">
          <div>
            ...
            <p className="mt-2 max-w-sm text-sm leading-6 text-muted-foreground">
              {query || tab !== 'all'
                ...
                : 'Start with a focused home for your Pages, plans, and collaborators.'}
            </p>
            {!query && tab === 'all' ? (
              <button
                type="button"
