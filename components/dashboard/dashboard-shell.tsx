...
const menuGroups: { label: string; items: MenuItem[] }[] = [
  {
    label: "Workspace",
    items: [
      { href: "/dashboard", label: "Dashboard", icon: LayoutDashboard, iconClassName: "text-[#146cf5] bg-[#e5efff]" },
      { href: "/dashboard/ai-assistant", label: "AI Assistant", icon: Bot, iconClassName: "text-[#7443e5] bg-[#eee8ff]" },
    ],
  },
  {
    label: "Plan",
    items: [
      { href: "/dashboard/calendar", label: "Calendar", icon: CalendarDays, iconClassName: "text-[#00a682] bg-[#def9f1]" },
      { href: "/dashboard/tasks", label: "Task / Kanban", icon: KanbanSquare, iconClassName: "text-[#f08725] bg-[#fff0df]" },
    ],
  },
  {
    label: "Create",
    items: [
      { href: "/dashboard/notes", label: "Notes", icon: NotebookPen, iconClassName: "text-[#d94e83] bg-[#ffebf3]" },
      { href: "/dashboard/whiteboard", label: "Whiteboard", icon: PanelsTopLeft, iconClassName: "text-[#008fbd] bg-[#e2f7ff]" },
      { href: "/dashboard/spaces", label: "Pages / Spaces", icon: FileStack, iconClassName: "text-[#719600] bg-[#f0f8d8]" },
    ],
  },
  {
    label: "Build with AI",
    items: [
      { href: "/dashboard/templates", label: "AI Template Builder", icon: BrainCircuit, iconClassName: "text-[#994dd8] bg-[#f6e9ff]" },
    ],
  },
  {
    label: "System",
    items: [
      { href: "/dashboard/settings", label: "Settings", icon: Settings2, iconClassName: "text-[#607187] bg-[#edf1f5]" },
    ],
  },
];
...
function NavContent({ collapsed, isDark, onNavigate, onToggleTheme }: { collapsed: boolean; isDark: boolean; onNavigate?: () => void; onToggleTheme: () => void }) {
  ...
  return (
    <>
      <div className={`flex h-[64px] items-center border-b border-[#dfe8f6] bg-[linear-gradient(135deg,#ffffff_0%,#f4f8ff_100%)] px-3 dark:border-white/10 dark:bg-none ${collapsed ? "justify-center" : "gap-2"}`}>
        <Link href="/dashboard" onClick={onNavigate} className="flex min-w-0 items-center gap-2.5" title="Flowbase dashboard">
          <span className="grid size-9 shrink-0 place-items-center rounded-xl bg-[#2468e5] text-white shadow-[0_8px_18px_rgba(36,104,229,0.22)]"><Sparkles size={17} fill="currentColor" strokeWidth={1.8} /></span>
          {!collapsed ? <span className="truncate text-[17px] font-semibold tracking-[-0.045em] text-[#1d2635] dark:text-[#f6f8fc]">flowbase</span> : null}
        </Link>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-3" aria-label="Workspace navigation">
        {menuGroups.map((group, groupIndex) => (
          <div key={group.label} className={groupIndex ? "mt-4" : ""}>
            {!collapsed ? <p className="mb-1 px-2 text-[9px] font-bold uppercase tracking-[0.16em] text-[#8b9eb9] dark:text-[#788397]">{group.label}</p> : <div className="mx-2 mb-1.5 h-px bg-[#e7eef8] dark:bg-white/10" />}
            <div className="space-y-0.5">
              {group.items.map(({ href, label, icon: Icon, iconClassName }) => {
                const active = isActiveRoute(pathname, href);
                return (
                  <Link key={href} href={href} onClick={onNavigate} title={collapsed ? label : undefined} className={`group flex h-8 items-center rounded-md px-2 text-[12px] font-medium transition duration-200 ${collapsed ? "justify-center" : "gap-2"} ${active ? "bg-[#e4efff] text-[#1461d2] shadow-[inset_0_0_0_1px_rgba(93,151,246,0.16)] dark:bg-[#1c3a70] dark:text-[#dce9ff]" : "text-[#68778b] hover:bg-[#eef5ff] hover:text-[#174da8] dark:text-[#aeb7c6] dark:hover:bg-white/[0.07] dark:hover:text-[#f4f6fa]"}`}>
                    <span className={`grid size-[22px] shrink-0 place-items-center rounded-md transition group-hover:scale-105 ${iconClassName} ${active ? "ring-1 ring-[#a9cafb]" : ""}`}><Icon size={13} strokeWidth={2.1} /></span>
      ...
      <div className="border-t border-[#e8edf4] p-2 dark:border-white/10">
        <Link href="/dashboard/settings" onClick={onNavigate} title={collapsed ? "Help & feedback" : undefined} className={`flex h-8 items-center rounded-md px-2 text-[12px] font-medium text-[#68778b] transition hover:bg-[#eef5ff] hover:text-[#174da8] dark:text-[#aeb7c6] dark:hover:bg-white/[0.07] dark:hover:text-[#f4f6fa] ${collapsed ? "justify-center" : "gap-2"}`}>
