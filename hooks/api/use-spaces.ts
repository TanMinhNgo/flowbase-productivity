...
export const spaceKeys = {
  ...
  page: (spaceId: number, pageId: number) =>
    ['spaces', spaceId, 'pages', pageId] as const,
};

export function useSpaces<T>() {
  return useQuery({
    queryKey: spaceKeys.all,
    queryFn: () => apiRequest<T>('/api/spaces'),
  });
}
...
export function useSpace<T>(spaceId: number) {
  return useQuery({
    ...
    queryFn: () => apiRequest<T>(`/api/spaces/${spaceId}`),
...
export function useCreateSpace<TItem, TPayload>() {
  ...
  return useMutation({
    mutationFn: (payload: TPayload) =>
      apiRequest<{ item: TItem }>('/api/spaces', {
        method: 'POST',
...
export function useUpdateSpace<TItem>() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      apiRequest<{ item: TItem }>(`/api/spaces/${spaceId}`, {
        method: 'PATCH',
...
export function useDeleteSpace() {
  ...
  return useMutation({
    mutationFn: (spaceId: number) =>
      apiRequest(`/api/spaces/${spaceId}`, { method: 'DELETE' }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: spaceKeys.all }),
  });
}
...
export function useCreateSpacePage<TItem>() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      apiRequest<{ item: TItem }>(`/api/spaces/${spaceId}/pages`, {
        method: 'POST',
...
export function useUpdateSpacePage<TItem>() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      apiRequest<{ item: TItem }>(`/api/spaces/${spaceId}/pages/${pageId}`, {
        method: 'PATCH',
        body: JSON.stringify(body),
      }),
    onSuccess: (_data, variables) =>
      Promise.all([
        ...
        queryClient.invalidateQueries({
          queryKey: spaceKeys.page(variables.spaceId, variables.pageId),
        }),
        queryClient.invalidateQueries({ queryKey: spaceKeys.all }),
      ]),
  });
...
export function useDeleteSpacePage() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ spaceId, pageId }: { spaceId: number; pageId: number }) =>
      apiRequest(`/api/spaces/${spaceId}/pages/${pageId}`, {
        method: 'DELETE',
      }),
    onSuccess: (_data, variables) =>
      Promise.all([
...
export function useInviteSpaceCollaborator() {
  ...
  return useMutation({
    mutationFn: ({ spaceId, email }: { spaceId: number; email: string }) =>
      apiRequest(`/api/spaces/${spaceId}/collaborators`, {
        method: 'POST',
